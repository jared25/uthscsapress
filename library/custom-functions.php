 ...
 */
 ...
 function utPress_register_meta_boxes() {
	add_meta_box( 
	'meta-box-id',
	__( 'Page Sub-Header Text', 'textdomain' ),
	'utPress_my_display_callback',
	'page', 
	'side' 
	);
}
add_action( 'add_meta_boxes', 'utPress_register_meta_boxes' );
 ...
 */
function utPress_my_display_callback( $post ) {
    // Display code/markup goes here. Don't forget to include nonces!
	wp_nonce_field( basename( __FILE__ ), 'utPress_nonce' );
    $utPress_stored_meta = get_post_meta( $post->ID );
    ?>
 
    <p>
        <label for="meta-text" class="utPress-row-title"><?php _e( 'The text you type below will apear under the page title in the featured banner', 'utPress-textdomain' )?></label> 
		<br>
        <input style="width:100%; height: 6em; vertical-align:text-top; margin-top:2em;" type="text" name="meta-text" value="<?php if ( isset ( $utPress_stored_meta['meta-text'] ) ) echo $utPress_stored_meta['meta-text'][0]; ?>" />
    </p>
    <?php
}
